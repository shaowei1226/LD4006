...
namespace EFC.Vision.Halcon
{
    public partial class TForm_Align_Mothed : Form
    {
        ...
        public void Update_View()
        {
            ...
            if (true)//jjs_hw.Init)
            {
                ...
                if (Step >= 1 && flag)
                {
                    if (!TJJS_Vision.Is_Empty(Image2)) image1 = Image1.Clone();
                ...
                if (Step >= 3 && flag)
                {
                    if (RB_Base_Image.Checked)
                    {
                        ...
                        Param.Find_Result.Draw_Point(JJS_HW.HW_Buf);
                    }
                    else
                    {
                        TAlign_Mothed_1.Trans(Image2, Param, ref Result);
                        image2.DispObj(JJS_HW.HW_Buf.HalconWindow);
                        JJS_HW.HW_Buf.HalconWindow.SetColor("red");
                        Param.Test_Region.Region.DispObj(JJS_HW.HW_Buf.HalconWindow);
                        Result.Draw_Point(JJS_HW.HW_Buf);
                    }
                }
                #endregion
    ...
    public class TAlign_Mothed_1
    {
        ...
        public static bool Set_Param(HImage image2, ref TAlign_Mothed_1_Param param)
        {
            bool result = false;
        ...
        public static bool Trans(HImage image, TAlign_Mothed_1_Param param, ref TAlign_Mothed_1_Result result_p)
        {
            bool result = false;
            HImage tmp_image = new HImage();

            if (param != null && !TJJS_Vision.Is_Empty(image))
            {
                try
                {
                    tmp_image = image.ReduceDomain(param.Test_Region.Region);
                    tmp_image.PointsHarris(param.Sigma_Grad, param.Sigma_Smooth,
                                            param.Alpha, param.Point_Num, out result_p.Row, out result_p.Col);
                    result_p.HomMat2D = tmp_image.ProjMatchPointsRansac(
                                                 param.Base_Image, result_p.Row, result_p.Col, param.Find_Result.Row, param.Find_Result.Col,
                                                 "ssd", 10, 0, 0, 255, 255, 0, 10, "normalized_dlt", 0.2, 0,
                                                 out result_p.Points, out param.Find_Result.Points);

                    result_p.Image = image.ProjectiveTransImage(result_p.HomMat2D, "nearest_neighbor", "false", "true");
                    result = true;
                }
                catch
                {
    ...
    public class TAlign_Mothed_1_Param
    {
        ...
        public TAlign_Mothed_1_Result Find_Result = new TAlign_Mothed_1_Result();
        ...
        public TAlign_Mothed_1_Param Copy()
        {
            ...
            result.Find_Result = Find_Result;
            return result;
        }
        ...
        public bool Update()
        {
            ...
            try
            {
                if (!TJJS_Vision.Is_Empty(Base_Image))
                {
                    tmp_image = Base_Image.ReduceDomain(Test_Region.Region);
                    tmp_image.PointsHarris(Sigma_Grad, Sigma_Smooth,
                                            Alpha, Point_Num, out Find_Result.Row, out Find_Result.Col);
                    Param_Data_OK = true;
                }
            }
            catch { }
    ...
    public class TAlign_Mothed_1_Result
    {
        ...
        public void Draw_Point(HWindowControl hw)
        {
            hw.HalconWindow.SetColor("green");
            hw.HalconWindow.SetLineWidth(2);
            hw.HalconWindow.DispCross(Row, Col, 10.0, 0.0);
        }
        public void Resset()
        {
            Find_OK = false;
