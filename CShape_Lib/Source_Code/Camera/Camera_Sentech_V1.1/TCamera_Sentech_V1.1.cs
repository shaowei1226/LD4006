    ...
    public class TCamera_Sentech_Giga : TCamera_Base
    {
        ...
        public void OnNodeCallback(INode node, object[] param)
        {
            ...
            if (node.IsAvailable)
            {
                ...
                {
                    Log_Add(fun, "OnNodeEvent:" + node.DisplayName + " : Invalidated", emLog_Type.Error);
                }
            }
        }
        private void OnCallback(IStCallbackParamBase paramBase, object[] param)
        {
            string fun = "OnCallback";
        ...
        public void Free()
        {
            ...
            {
                Log_Add(fun, "[Free]" + e.Message, emLog_Type.Error);
            }
        }
        override public void Camera_Init()
        {
            string fun = "Camera_Init";

            Log_Add(fun, "Camera_Init Start.");
            try
            {
                if (Device != null)
                {
            ...
            {
                Log_Add(fun, "[Camera_Init]" + e.Message, emLog_Type.Error);
            }
        ...
        private void Set_EventDeviceLost(NodeEventHandler handler)
        {
            ...
            nodeCallback.RegisterCallbackMethod(handler, param, eCallbackType.PostOutsideLock);
            Device.StartEventAcquisitionThread();
        }
        private void On_Disconnected()
        {
            string fun = "On_Disconnected";

            Log_Add(fun, "[On_Disconnected].", emLog_Type.Error);
            Gen_Empty_Image();
        ...
        {
            string fun = "On_Reconnected";

            Timer_Reconnect.Enabled = false;
            Log_Add(fun, "[On_Reconnected].");
            Free();
            ...
            if (FInit)
            {
                Log_Add(fun, "[On_Reconnected] OK.");
            }
            else
            {
                Log_Add(fun, Name + "[On_Reconnected] NG.", emLog_Type.Error);
                Timer_Reconnect.Enabled = true;
            }
        }
